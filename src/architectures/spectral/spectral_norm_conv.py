...
class SpectralNormConvTransposed(SpectralNorm):
    def compute_weight(self, module, do_power_iteration: bool) -> torch.Tensor:
        weight = getattr(module, self.name + "_orig")
        u = getattr(module, self.name + "_u")
        v = getattr(module, self.name + "_v")
    ...
    def apply(module, coeff, input_dim, name, n_power_iterations, eps):
        for k, hook in module._forward_pre_hooks.items():
            if isinstance(hook, SpectralNormConvTransposed) and hook.name == name:
                raise RuntimeError(
                    "Cannot register two spectral_norm hooks on "
                    "the same parameter {}".format(name)
                )

        fn = SpectralNormConvTransposed(name, n_power_iterations, eps=eps)
        fn.coeff = coeff
        fn.input_dim = input_dim
        weight = module._parameters[name]
...
def spectral_norm_conv_transposed(
    module, coeff, input_dim, n_power_iterations=1, name="weight", eps=1e-12,
):
    """
    Applies spectral normalization to Convolutions with flexible max norm
    ...
    Example::
        ...
        >>> m = spectral_norm_conv(nn.ConvTransposed2D(3, 16, 3), (3, 32, 32), 2.0)

    """

    input_dim_4d = torch.Size([1, input_dim[0], input_dim[1], input_dim[2]])
    SpectralNormConvTransposed.apply(module, coeff, input_dim_4d, name, n_power_iterations, eps)
